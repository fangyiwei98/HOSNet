...
class MyNet(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(MyNet, self).__init__(init_cfg)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        loss_seg_t2t, log_vars_seg_t2t = self._get_segmentor_loss(self.decode_head_t, P_t2s, pseudo_label,gt_weight=pseudo_weight)
    ...
    def compute_distance_loss(self,features, prototypes, num_classes, dev=None):
        """
        计算特征图与原型之间的平均距离损失。

        参数:
            features (Tensor): 预测的特征图，形状为 [batch_size, channels, height, width]。
            prototypes (Tensor): 原型矩阵，其中每一行是一个原型向量，形状为 [num_classes, channels]。
            num_classes (int): 类别的数量。
            dev (str or torch.device, optional): 设备指定，例如 'cuda' 或 'cpu'。

        返回:
            loss (Tensor): 计算的平均距离损失值。
        """
        # 确保原型和特征图在相同的设备上
        if dev is not None:
        ...
        distances = torch.zeros(batch_size, num_classes, height, width, device=features.device)

        # 计算每个像素与每个原型之间的距离
        for cls, prototype in enumerate(prototypes):
            distances[:, cls, :, :] = (
                (features - prototype.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)).pow(2).sum(dim=1)).sqrt()

            # 对每个像素点取距离最近的原型所对应的距离
    ...
    def compute_class_weights(self, features, labels, num_classes, temp=1.0):
        """
        计算基于每个像素到其最近原型的距离的权重。

        参数:
            features (Tensor): 预测的特征图，形状为 [N, C, H, W]。
            labels (Tensor): 类别标签，形状为 [N, 1, H, W]。
            num_classes (int): 类别的数量。
            temp (float, optional): 控制权重分布的温度参数。

        返回:
            weights (Tensor): 形状为 [N, num_classes, H, W] 的权重张量。
        """
        batch_size, channels, height, width = features.shape
        # 确保features和labels在相同的设备上
        device = features.device
        labels = labels.to(device)

        # 初始化距离张量
        distances = torch.full((batch_size, num_classes, height, width), float('inf'), device=device)

        # 计算每个像素与每个原型之间的距离
        for cls in range(num_classes):
            prototype = self.prototypes[cls]  # 假设self.prototypes包含预计算的原型
            diff = features - prototype.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            distances[:, cls, :, :] = diff.pow(2).sum(dim=1).sqrt()

            # 对每个像素取距离最近的原型所对应的距离，并计算权重
        min_distances, min_indices = torch.min(distances, dim=1)
        # 使用负指数函数计算权重
        weights_unnormalized = torch.exp(-min_distances / temp)

        # 沿着类别维度对权重进行softmax归一化（可选）
        weights = F.softmax(weights_unnormalized.unsqueeze(1), dim=2).squeeze(1)

        # 如果需要，可以将权重重塑为 [N, C, H, W] 的形状，其中 C == num_classes
        # 但通常我们保持 [N, num_classes, H, W] 的形状，以便可以轻松地选择每个类别的权重
        # weights = weights.view(batch_size, num_classes, height, width)  # 如果确实需要这种形状的话
