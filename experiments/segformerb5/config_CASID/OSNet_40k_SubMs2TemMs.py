_base_ = [
    '../../../configs/_base_/datasets/SubMs2TemMs.py',
...
model = dict(
    ...
    decode_head_t=dict(
        ...
        #sampler=dict(type='OHEMPixelSampler', thresh=0.7, min_kept=100000),
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[1.0, 1.0, 1.25, 1.25, 1.5])),
    ...
    cross_EMA = dict(
        ...
        pseudo_class_weight=[1.01, 1.01, 1.51, 1.51, 2.01],
        backbone_EMA=dict(
            type='MixVisionTransformer',
            init_cfg=dict(type='Pretrained', checkpoint='./pretrained/mit_b5.pth'),
            in_channels=3,
